...
def make_figure2(history, geometry, H_threshold, T_orb, n_orbits, path):
    ...
    ax.axhline(H_threshold, color="black", linestyle="--", linewidth=1.2, label=r"$+H_{threshold}$")
    ax.axhline(-H_threshold, color="black", linestyle="--", linewidth=1.2, label=r"$-H_{threshold}$")
...
def make_figure3(g3, g4, env, T_orb, H_threshold, path):
    ...
    ax.axhline(1.0, color="black", linestyle="--", linewidth=1.0, label="threshold (rho_H=1)")
    ax.set_xlabel("Orbits")
    ax.set_ylabel(r"$\rho_H(t) = \max_i |h_{w,i}(t)|/H_{threshold}$")
