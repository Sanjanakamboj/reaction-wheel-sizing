...
def make_figure1(dump, geometry, thresholds, path):
    ...
    ax.axhline(thresholds.H_on, color="red", linestyle="--", linewidth=1.2, label=r"$+H_{\rm on}$")
    ax.axhline(-thresholds.H_on, color="red", linestyle="--", linewidth=1.2)
    ax.axhline(thresholds.H_off, color="green", linestyle="--", linewidth=1.2, label=r"$+H_{\rm off}$")
...
def make_figure2(dump, B_field, path):
    ...
    ax0b = axs[0].twinx()
    l1, = axs[0].plot(t / 3600, np.linalg.norm(B_hist, axis=1) * 1e5, color="tab:blue",
                       label="Geomagnetic field magnitude |B(t)|")
    l2, = ax0b.plot(t / 3600, np.linalg.norm(dump.tau_achieved, axis=1) * 1e4, color="tab:red",
                     label="Total magnetic torque magnitude")
    axs[0].set_ylabel(r"$|B(t)|$ [$10^{-5}$ T]", color="tab:blue")
    axs[0].tick_params(axis="y", labelcolor="tab:blue")
    ax0b.set_ylabel(r"Torque magnitude [$10^{-4}$ N·m]", color="tab:red")
    ax0b.tick_params(axis="y", labelcolor="tab:red")
    axs[0].set_title("Field magnitude and total achieved magnetic torque magnitude")
    axs[0].legend(handles=[l1, l2], fontsize=8, loc="upper right")
    axs[0].grid(True, alpha=0.3)

    axs[1].plot(t / 3600, dump.effectiveness, color="tab:green")
    axs[1].set_ylabel(r"Useful fraction $\eta_B(t)$")
    axs[1].set_xlabel("Time [hours]")
    axs[1].set_title("Useful projection onto desired unloading direction (independent of dipole saturation)")
...
def make_figure3(events, g4, env, B_field, mt, k_H, thresholds, H_max, T_orb, dt_dump, t_max_dump, path):
    ...
    ax.axhline(thresholds.H_on / H_max, color="black", linestyle="--", linewidth=1.0, label=r"$H_{\rm on}/H_{\max}$")
    ax.axhline(thresholds.H_off / H_max, color="gray", linestyle="--", linewidth=1.0, label=r"$H_{\rm off}/H_{\max}$")
