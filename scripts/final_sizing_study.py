...
def make_figure2(time_results, H_on_old, H_max_old, path):
    ...
    ax.axhline(headroom, color="black", linestyle="--",
               label=fr"20% capacity headroom at $H_{{on}}=0.8\,H_{{max}}$: {headroom:.2f} N·m·s")
    ax.set_xlabel("Maneuver duration T [s]")
...
def make_figure3(torque_req, H_raw, old_wheel, final_wheel, path):
    ...
    ax.plot(old_wheel.tau_max, old_wheel.H_max, "k*", markersize=16,
            label="M1 synthetic wheel (evaluated against M5 design requirement)")
    ax.plot(final_wheel.tau_max, final_wheel.H_max, "b^", markersize=14,
            label="M5 final recommendation")
...
def make_figure4(trade_rows, Omega_max_recommended_rpm, J_w_recommended, path):
    ...
    ax.set_ylabel(r"Required rotor inertia $J_w$ [kg·m²]")
    ax.set_title(r"Figure 4 — Rotor Inertia vs Maximum Wheel Speed (for $H_{\rm rec}$)")
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.legend()
    ax.grid(True, alpha=0.3, which="both")
...
def make_figure5(dump_final, final_schedule, geometry, path):
    ...
    ax.axhline(final_schedule.H_on, color="red", linestyle="--", label=r"$+H_{\rm on}$ (final wheel)")
    ax.axhline(-final_schedule.H_on, color="red", linestyle="--")
    ax.axhline(final_schedule.H_off, color="green", linestyle="--", label=r"$+H_{\rm off}$ (final wheel)")
